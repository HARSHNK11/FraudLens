...
export default function TransactionForm({ onSubmit, loading, requiredFeatures }) {
  ...
  const featuresToRender = requiredFeatures || ALL_FEATURES;
  const isComplete = featuresToRender.every(f => values[f] !== '');
  
  const vFeaturesToRender = V_FEATURES.filter(feat => featuresToRender.includes(feat));
  const negativeFeatures = vFeaturesToRender.filter(feat => FRAUD_EXAMPLE[feat] < 0);
  const positiveFeatures = vFeaturesToRender.filter(feat => FRAUD_EXAMPLE[feat] >= 0);

  const handleSubmit = (e) => {
    e.preventDefault();
    const parsed = {};
    for (const k of featuresToRender) {
      const v = values[k];
    ...
    <div className="tx-form-wrapper">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="tx-special-row">
          {featuresToRender.includes('Time') && (
          ...
          {featuresToRender.includes('Amount') && (
            <div className="form-group">
        ...
        {negativeFeatures.length > 0 && (
          <>
            <div className="tx-pca-label" style={{ marginTop: '20px' }}>
              <span className="form-label" style={{ color: '#ef4444' }}>Negative Bias PCA Features</span>
              <span className="tx-pca-note">Values that are typically negative during fraud</span>
            </div>
            <div className="tx-grid">
              {negativeFeatures.map(feat => (
        ...
        {positiveFeatures.length > 0 && (
          <>
            <div className="tx-pca-label" style={{ marginTop: '20px' }}>
              <span className="form-label" style={{ color: '#10b981' }}>Positive Bias PCA Features</span>
              <span className="tx-pca-note">Values that are typically positive during fraud</span>
            </div>
            <div className="tx-grid">
        ...
        <div className="tx-progress">
          <div className="flex justify-between mb-1" style={{fontSize:'0.75rem',color:'var(--text-muted)'}}>
            ...
            <span>{featuresToRender.filter(f => values[f] !== '').length} / {featuresToRender.length}</span>
          </div>
          <div className="progress-bar">
            <div
              className="progress-fill"
              style={{
                width: `${(featuresToRender.filter(f => values[f] !== '').length / featuresToRender.length) * 100}%`,
